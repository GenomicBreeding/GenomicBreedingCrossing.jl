...
"""
    gpmodels(genomes::Genomes, phenomes::Phenomes; save::Bool=true) -> Dict{String,Fit}

Fit genomic prediction models for each trait in `phenomes` and return a mapping
from trait name to fitted model.

Description
- Iterates over each trait listed in `phenomes.traits`.
- For each trait:
  - Extracts the phenotype vector for that trait.
  - Filters observations to keep only finite, non-missing values.
  - Slices `phenomes` to the retained entries and the current trait.
  - Merges the (filtered) phenomes with `genomes` to produce inputs for modeling.
  - Calls `GenomicBreedingModels.bayesa(genomes=G, phenomes=P)` to fit the model.
- Collects the resulting `Fit` objects in a `Dict{String,Fit}` keyed by trait name.
- If `save` is true (default), the resulting dictionary is saved to "fits.jld2"
  using `JLD2.save("fits.jld2", fits)` as a side effect.

Arguments
- `genomes::Genomes`: Genotypic data structure expected by the merging and model functions.
- `phenomes::Phenomes`: Phenotypic data structure with a `traits` field and a `phenotypes`
  matrix or similar layout used to extract trait columns.
- `save::Bool=true`: Keyword argument. When `true`, persist the resulting dictionary to
  "fits.jld2". When `false`, do not write to disk.

Returns
- `Dict{String,Fit}`: A dictionary mapping each trait name (as a `String`) to the fitted
  `Fit` object returned by `GenomicBreedingModels.bayesa`.

Notes & Caveats
- The function expects at least one finite, non-missing observation per trait; if no
  observations remain after filtering, downstream operations (slicing, merging, or the
  `bayesa` call) may error.
- The exact behavior and requirements of `GenomicBreedingModels.bayesa`, `slice`,
  and `merge` depend on their implementations; ensure `G` and `P` conform to those APIs.
- The file "fits.jld2" will be overwritten when `save=true`.

Example
```jldoctest; setup=:(using GenomicBreedingCore, GenomicBreedingIO, GenomicBreedingModels, DataFrames, CSV, StatsBase, JLD2)
julia> true
true
```
"""
function gpmodels(genomes::Genomes, phenomes::Phenomes; save::Bool = true)::Dict{String,Fit}
    fits = Dict()
